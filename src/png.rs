...
impl PNG {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> CustomResult<Chunk> {
...
impl TryFrom<&[u8]> for PNG {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != PNG::STANDARD_HEADER {
            return Err("Bad header (received {:?}, expected {:?})".into());
        }
        ...
        while let Ok(()) = reader.read_exact(&mut length_buffer) {
            ...
            let mut buffer = vec![0; length + 8]; // 4 bytes for chunk type and 4 bytes for CRC
...
impl fmt::Display for PNG {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Convert header to a string of hexadecimal values
        let header_str = self
            .header
            .iter()
